// content.js - COMPLETE AUTO-SHORTS REMOVAL v7.0.0 - FIXED
(function() {
    'use strict';
    ...
    const CONFIG = {
        removeShorts: true,
        maxBlockedVideos: 5000,
        annotationRemoval: true,
        autoSkipAds: false // Disabled since we integrate with uBlock
    };
    ...
    function removeShortsElement(element) {
        try {
            ...
            // Try to extract actual Shorts title
    ...
    function extractVideoInfo(videoElement) {
        try {
            if (isShortsElement(videoElement)) {
                // Enhanced Shorts title extraction
                let shortsTitle = 'YouTube Shorts';
                
                // Try to get actual Shorts title from various selectors
                const titleSelectors = [
                    '#video-title',
                    'h3 a',
                    'a#video-title-link',
        ...
        try {
            switch (request.type) {
                ...
                case "saveEnhancedConfig":
                    // Update local config with enhanced settings
                    if (request.config) {
